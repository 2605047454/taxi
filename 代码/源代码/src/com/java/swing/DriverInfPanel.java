...
import com.java.dao.DriverInfDAO;
...
public class DriverInfPanel extends JPanel{
	...
	ArrayList<DriverInfEntity> list;
	public static DefaultTableModel tablemodel;
	public DriverInfPanel(){
		
		querypanel = new QueryPanel();
		String[] dataString = DriverInfEntity.title;
		...
		list = DriverInfDAO.findAll();
		for(DriverInfEntity temp : list){
			dataline = new Vector<String>();
			dataline.add(temp.getName());
			dataline.add(temp.getAddoilcount());
			dataline.add(temp.getRepaircount());
			dataline.add(temp.getBreakcount());
			dataline.add(temp.getScorecount());
			dataline.add(temp.getLeavecount());
			dataline.add(temp.getWorkcount());
			dataline.add(temp.getAddscorcount());
			//dataline.add(temp.);
			data.add(dataline);
		}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getSource()==save){
				if(JOptionPane.showConfirmDialog(this, "是否要保存此数据表格", "保存数据", 2)==JOptionPane.YES_OPTION){
					...
					TableToWord.openWord("carDate\\人员综合信息表"+ temp +".doc", DriverInfEntity.width,"人员综合信息表", DriverInfEntity.title.length, DriverInfEntity.title, data,0,0);
					try {
						String a ="carDate\\人员综合信息表"+ temp +".doc";
